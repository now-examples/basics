...
async function main(req, res, parsedUrl) {
  // Redirect to /examples as that's the main page for this zone
  if (req.url === '/') {
  ...
  const shouldntCache =
    dev || (req.headers.cookie || '').includes('token=') || isNext
  ...
  if (match) {
    return app.render(req, res, '/example', { example: match[1] })
  }
